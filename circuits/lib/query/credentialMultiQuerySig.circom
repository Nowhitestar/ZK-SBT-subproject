...
include "iden3-circuits/lib/query/comparators.circom";
include "iden3-circuits/lib/idOwnershipBySignature.circom";
include "iden3-circuits/lib/query/query.circom";
include "circuits/lib/utils/claimUtils.circom"


/**
credentialMultiQuerySig.circom - query claim value and verify claim issuer signature:
...
*/
template CredentialMultiQuerySig(IdOwnershipLevels, IssuerLevels, valueArraySize, IndexNum) {
    ...
    */
    ...
    signal input slotIndex[IndexNum];
    signal input operator[IndexNum];
    ...
    */
    ...
    // query
    component getClaimValue = getMultiValueByIndex(IndexNum);
    for (var i=0; i<8; i++) { getClaimValue.claim[i] <== issuerClaim[i]; }
    for (var i=0; i<IndexNum, i++) { getClaimValue.index[i] <== slotIndex[i];}

    for (var i=0, i<IndexNum, i++) {
        component q[num] = Query(valueArraySize);
        q[num].in <== getClaimValue.value[num];
        q.operator <== operator[num];
        for(var i = 0; i<valueArraySize; i++){q.value[i] <== value[i];}
        q[num].out === 1;
    }
    var q_final = 1;
    for (var i=0, i<IndexNum, i++) {
        q_final = q[num]*q_final
    }
    q_final === 1;
}
